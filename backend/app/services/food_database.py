...
Contains nutritional info per 100g for common foods,
search functionality, and nutrition calculation.
Migrated from frontend food-database.js.
"""
...
def search_food(food_name: str) -> Optional[dict]:
    ...
    Returns dict with {name, calories, protein, carbs, fat} or None.
...
def calculate_nutrition(food_data: dict, grams: float) -> dict:
    ...
    food_data should have: calories, protein, carbs, fat (per 100g).
    Returns dict with: name, grams, calories, protein, carbs, fat.
    """
    ratio = grams / 100
    return {
        "name": food_data["name"],
        ...
        "protein": round(food_data["protein"] * ratio * 10) / 10,
        "carbs": round(food_data["carbs"] * ratio * 10) / 10,
        "fat": round(food_data["fat"] * ratio * 10) / 10,
    }


def get_all_foods() -> list[dict]:
    """Return all foods in the database as a list."""
    return [{"name": name, **data} for name, data in FOOD_DATABASE.items()]
